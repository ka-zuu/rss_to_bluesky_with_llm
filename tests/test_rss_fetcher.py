...
def test_fetch_new_articles_with_new_and_old_entries(mocker):
    ...
    assert new_articles[1]["title"] == "New Article 2"
...
def test_fetch_new_articles_with_only_new_entries(mocker):
    ...
    mock_entries = [
        MockEntry("New Article 1", "http://example.com/new1", "Summary 1"),
        MockEntry("New Article 2", "http://example.com/new2", "Summary 2"),
    ]
    mocker.patch("rss_fetcher.feedparser.parse", return_value=MockFeed(mock_entries))
...
def test_fetch_from_multiple_feeds(mocker):
    """複数のRSSフィードから取得するテスト"""
    feed1_entries = [MockEntry("Feed 1 Article", "http://f1.com/a1", "S1")]
    feed2_entries = [MockEntry("Feed 2 Article", "http://f2.com/a2", "S2")]
    ...
    assert len(new_articles) == 2
    assert new_articles[0]["link"] == "http://f1.com/a1"
    assert new_articles[1]["link"] == "http://f2.com/a2"
